...
"use strict";let $;addStyles(),document.addEventListener("turbo:load",()=>{$??=unsafeWindow.jQuery,$&&(unsafeWindow.ratingOverlay=ratingOverlay,addLinksToPosters(),$(document).off("ajaxSuccess.userscript12944").on("ajaxSuccess.userscript12944",(t,e,r)=>{r.url.endsWith("/popular_lists")&&(addLinksToPosters(),unsafeWindow.addOverlays())}))},{capture:!0});function ratingOverlay(t,e){if(!t.length){const r=t.end();r.closest(".personal-list").length&&r.hasClass("poster")&&(t=r)}t.find(".corner-rating").length||t.prepend(`<div class="corner-rating corner-rating-${e}"><div class="text">${e}</div></div>`)}function addLinksToPosters(){$(".personal-list .poster[data-url]:not(:has(> a))").each(function(){$(this).children().wrapAll(`<a href="${$(this).attr("data-url")}"></a>`)})}unsafeWindow.userscriptAddLinksToListPreviewPosters=addLinksToPosters;function addStyles(){GM_addStyle(`
