...
"use strict";let $,toastr;addStyles(),document.addEventListener("turbo:load",()=>{if(!/^\/people\/[^\/]+$/.test(location.pathname)||($??=unsafeWindow.jQuery,toastr??=unsafeWindow.toastr,!$||!toastr))return;let e;$('<button id="btn-pronounce-name"><div class="audio-animation fade"><div class="bar-1"></div><div class="bar-2"></div><div class="bar-3"></div></div><div class="fa fa-volume fade in"></div></button>').appendTo($("#summary-wrapper .mobile-title h1")).tooltip({title:"Pronounce Name",container:"body",placement:"top",html:!0}).on("click",async function(){if($(this).tooltip("hide"),!e){unsafeWindow.showLoading?.();const t=$('body > [itemtype$="Person"] > meta[itemprop="name"]').attr("content"),o=await GM.xmlHttpRequest({url:`https://forvo.com/search/${encodeURIComponent(t)}`}),a=new DOMParser().parseFromString(o.responseText,"text/html"),i=$(a).find("body > script").text().match(/_AUDIO_HTTP_HOST='(.+?)'/)?.[1],n=$(a).find('[onclick^="Play"]').attr("onclick")?.match(/Play\([0-9]+,'(.*?)','(.*?)',(?:true|false),'(.*?)','(.*?)'/)?.slice(1).map(atob);if(unsafeWindow.hideLoading?.(),!n?.length){toastr.error(`Userscript | Actor Pronunciation Helper: Could not find a pronunciation for ${t} on forvo.com`);return}const r=n[0]?`/mp3/${n[0]}`:null,s=n[1]?`/ogg/${n[1]}`:null,l=n[2]?`/audios/mp3/${n[2]}`:null,d=n[3]?`/audios/ogg/${n[3]}`:null;e??=new Audio("https://"+i+(d??l??s??r)),$(e).off("ended").on("ended",()=>{setTimeout(()=>{$(this).find(".audio-animation").removeClass("in"),setTimeout(()=>$(this).find(".fa").addClass("in"),100)},100)})}$(this).find(".fa").removeClass("in"),setTimeout(()=>{$(this).find(".audio-animation").addClass("in"),e.load(),e.play()},200)})},{capture:!0});function addStyles(){GM_addStyle(`
    #btn-pronounce-name {
      margin: 0 0 2px 7px;
      position: relative;
      height: 20px;
