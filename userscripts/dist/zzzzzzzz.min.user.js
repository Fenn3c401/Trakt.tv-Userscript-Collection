...
`),(r=>document.readyState==="loading"?document.addEventListener("DOMContentLoaded",r):r())(()=>{if(!unsafeWindow.jQuery)return;const r=Object.getOwnPropertyDescriptors(unsafeWindow.jQuery.fn.tooltip),f=Object.getOwnPropertyDescriptor(unsafeWindow.jQuery.fn,"tooltip"),v=f.value;f.value=function(...c){return c[0]?.container&&this.closest(".popover, #ondeck-wrapper, #progress-grid-wrapper").length&&delete c[0].container,v.apply(this,c)},Object.defineProperty(unsafeWindow.jQuery.fn,"tooltip",f),Object.entries(r).forEach(([c,m])=>!["length","name","prototype"].includes(c)&&Object.defineProperty(unsafeWindow.jQuery.fn.tooltip,c,m))}),GM_addStyle(`
@media (width <= 767px) {
...
`),window.addEventListener("turbo:load",()=>{const r=unsafeWindow.jQuery("body.touch-device #info-wrapper:has(.sidebar)");r.swipe({excludedElements:"#summary-ratings-wrapper .stats, #info-wrapper .season-links .links, #actors .posters",swipeRight:(f,v,c,m,o,n)=>n[0].start.x<50&&r.addClass("with-mobile-sidebar"),swipeLeft:(f,v,c,m,o,n)=>r.removeClass("with-mobile-sidebar")})}),window.addEventListener("turbo:load",()=>{document.querySelectorAll("#header-search-type .dropdown-menu li:has(~ .divider) a").forEach((r,f)=>{unsafeWindow.Mousetrap.bind(`alt+${f+1}`,()=>r.click()),unsafeWindow.Mousetrap(document.getElementById("header-search-query")).bind(`alt+${f+1}`,()=>r.click())})}),(r=>document.readyState==="loading"?document.addEventListener("DOMContentLoaded",r):r())(()=>{const r=unsafeWindow.jQuery;r&&(r(document).on("auxclick",".btn-watch .view-all",function(f){f.preventDefault(),GM_openInTab(location.origin+r(this).attr("data-url"),{setParent:!0})}),r(document).on("mousedown mouseup","#header-search-autocomplete-results .selected",function(f){f.which===2&&!r(f.target).closest("a").length&&(f.type==="mousedown"?f.preventDefault():(unsafeWindow.searchModifierKey=!0,r(this).trigger("click")))}))}),document.addEventListener("keydown",r=>{r.ctrlKey&&r.key==="Enter"&&r.target.closest?.("#header-search-query")&&(r.preventDefault(),r.stopPropagation(),r.target.dispatchEvent(new KeyboardEvent("keydown",{key:"Enter",keyCode:13,metaKey:!0,bubbles:!0,cancelable:!0})))},{capture:!0}),(r=>document.readyState==="loading"?document.addEventListener("DOMContentLoaded",r):r())(()=>{const r=unsafeWindow.jQuery;r&&r(document).on("ajaxSuccess",(f,v,c)=>{if(c.url.endsWith("/rate")){const m=new URLSearchParams(c.data),[o,n,d]=["type","trakt_id","stars"].map(s=>m.get(s));unsafeWindow[o+"s"].ratings[n]=d,unsafeWindow.compressedCache.set(`ratings_${o}s`,unsafeWindow[o+"s"].ratings),unsafeWindow.addOverlays()}else if(c.url.endsWith("/rate/remove")){const m=new URLSearchParams(c.data),o=m.get("type");unsafeWindow.compressedCache.set(`ratings_${o}s`,unsafeWindow[o+"s"].ratings),unsafeWindow.addOverlays()}})}),GM_addStyle(`
.personal-list .list-description {
  overflow-wrap: anywhere;
}
`),(r=>document.readyState==="loading"?document.addEventListener("DOMContentLoaded",r):r())(()=>{if(!unsafeWindow.jQuery)return;const r=Object.getOwnPropertyDescriptors(unsafeWindow.jQuery.fn.chosen),f=Object.getOwnPropertyDescriptor(unsafeWindow.jQuery.fn,"chosen"),v=f.value;f.value=function(...c){if(this.attr("id")==="filter-network_ids"&&(c[0].max_shown_results=200),/iP(od|hone)|IEMobile|Windows Phone|BlackBerry|BB10|Android.*Mobile/i.test(unsafeWindow.navigator.userAgent)){Object.defineProperty(unsafeWindow.navigator,"userAgent",{get:()=>"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36",configurable:!0});try{return v.apply(this,c)}finally{delete unsafeWindow.navigator.userAgent}}else return v.apply(this,c)},Object.defineProperty(unsafeWindow.jQuery.fn,"chosen",f),Object.entries(r).forEach(([c,m])=>!["length","name","prototype"].includes(c)&&Object.defineProperty(unsafeWindow.jQuery.fn.chosen,c,m))}),(r=>document.readyState==="loading"?document.addEventListener("DOMContentLoaded",r):r())(()=>{const r=unsafeWindow.jQuery;r&&r(document).on("ajaxSend",(f,v,c)=>{if(/\/lists\/[\d]+\/like/.test(c.url)){const m=new URLSearchParams(c.data).get("trakt_id"),o=r(`[data-list-id="${m}"] > .like .count-number`),n=o.text(),d=c.url.includes("/remove");r(document).one("ajaxSuccess",(s,g,l)=>{c.url===l.url&&o.text(unsafeWindow.numeral(n)[d?"subtract":"add"](1).format("0,0"))})}})}),GM_addStyle(`
...
`);const t=()=>{unsafeWindow.jQuery('.readmore:not([id^="rmjs-"])').filter((f,v)=>unsafeWindow.jQuery(v).height()>350).readmore({embedCSS:!1,collapsedHeight:300,speed:200,moreLink:'<a href="#">Read more...</a>',lessLink:'<a href="#">Read less...</a>',afterToggle:(f,v,c)=>v.closest("#sortable-grid").length&&unsafeWindow.$grid?.isotope()}),requestAnimationFrame(()=>unsafeWindow.$grid?.isotope())};Object.defineProperty(unsafeWindow,"renderReadmore",{get:()=>t,set:()=>{},configurable:!0}),GM_addStyle(`
...
`),(r=>document.readyState==="loading"?document.addEventListener("DOMContentLoaded",r):r())(()=>{unsafeWindow.jQuery?.(document).on("ajaxSuccess",(r,f,v)=>{v.url.endsWith("/dashboard/schedule")&&unsafeWindow.jQuery("#schedule-wrapper .btn-watch-now:not([data-source-counts])").attr("data-source-counts","{}"),/\/(dashboard\/on_deck|progress_item\/watched)\/\d+$/.test(v.url)&&unsafeWindow.posterGridTooltips?.()})}),GM_addStyle(`
body {
  overflow-x: clip !important;
}
`),GM_addStyle(`
...
`)})("Trakt.tv | Bug Fixes And Optimizations"),gmStorage.cs1u5z40&&(async C=>{"use strict";let t,r,f,v,c,m,o,n,d;Chart.defaults.borderColor="rgb(44 44 44 / 0.5)";const s=new Intl.NumberFormat("en",{notation:"compact",maximumFractionDigits:1});s.formatTLC=p=>s.format(p).toLowerCase(),k(),document.addEventListener("turbo:load",async()=>{/^\/shows\/[^/]+\/seasons\/[^/]+$/.test(location.pathname)&&(t??=unsafeWindow.jQuery,r??=unsafeWindow.userscriptTraktApiWrapper,t&&(f=t("#seasons-episodes-sortable"),f.length&&(v=location.pathname.includes("/seasons/"),c=!location.pathname.includes("/seasons/0"),m=v?p=>`${p.seasonNum}x${String(p.episodeNum).padStart(2,"0")} ${p.watched?"\u2714":"\u2718"}`:p=>`S. ${p.seasonNum} ${p.watched?p.watched==100?"\u2714":`(${p.watched}%)`:"\u2718"}`,o=null,n=[],d=!0,!(!v&&+t(".season-count").text().split(" ")[0]<4||location.pathname.includes("/alternate/")&&location.pathname.split("/").filter(Boolean).length<6)&&(f.on("arrangeComplete",()=>{f.data("isotope")&&(o?l():g())}),t(document).off("ajaxSuccess.userscript48372").on("ajaxSuccess.userscript48372",(p,w,e)=>{e.url.includes("/rate")&&o&&l()})))))},{capture:!0});function g(){const p=t('<div id="seasons-episodes-chart-wrapper"><canvas></canvas></div>').insertBefore(f).children()[0];o=new Chart(p.getContext("2d"),{type:"line",data:_(),options:y()});const w=new IntersectionObserver(e=>{e.forEach(a=>{a.isIntersecting&&(w.disconnect(),document.hidden?t(document).one("visibilitychange",l):l())})},{threshold:1});w.observe(p),p.addEventListener("click",e=>{const a=o.getElementsAtEventForMode(e,"nearest",{axis:"x",intersect:!1},!0);if(!a.length)return;const i=a.sort((u,h)=>Math.abs(u.element.y-e.layerY)-Math.abs(h.element.y-e.layerY))[0];if(Math.abs(i.element.y-e.layerY)<10){const u=`${n[i.index].urlFull}${i.datasetIndex===3?"/comments":""}`;GM_openInTab(u,{active:!0})}else o.isZoomedOrPanned()&&o.resetZoom("active")})}async function l(){const p=await b();JSON.stringify(n)!==JSON.stringify(p)&&(n=p,o.data=_(),o.options=y(),o.update(),d&&(d=!1))}function b(){const p=f.data("isotope").filteredItems.filter(w=>c?w.element.dataset.seasonNumber!=="0":!0).map(async w=>{const e={generalRating:w.sortData.percentage,votes:w.sortData.votes,watchers:w.sortData.watchers,episodeNum:w.element.dataset.number||null,seasonNum:w.element.dataset.seasonNumber,urlFull:t(w.element).find('meta[itemprop="url"]').attr("content"),personalRating:t(w.element).find(".corner-rating > .text").text()||null,watched:t(w.element).find("a.watch.selected").attr("data-percentage")??0,releaseDate:t(w.element).find(".percentage").attr("data-earliest-release-date")};if(v)e.mainTitle=t(w.element).find(".under-info .main-title").text(),e.comments=t(w.element).find('.episode-stats > a[data-original-title="Comments"]').text()||0;else if(e.mainTitle=t(w.element).find('div[data-type="season"] .titles-link h3').text(),r)e.comments=(await r.seasons.comments({id:w.element.dataset.showId,season:e.seasonNum,limit:1})).item_count;else{const a=await fetch(w.element.dataset.url+".json");if(!a.ok)throw new Error(`XHR for: ${a.url} failed with status: ${a.status}`);e.comments=(await a.json()).stats.comment_count}return e});return Promise.all(v?p:p.reverse())}function x(p,w,e,...a){if(!p)return a.pop().color;const{ctx:i,chartArea:u,scales:h}=p.chart;if(u){if(i[w]??={},!i[w].gradient||i[w].height!==u.height||i[w].yAxisMin!==h[e].min||i[w].yAxisMax!==h[e].max){i[w].height=u.height,i[w].yAxisMin=h[e].min,i[w].yAxisMax=h[e].max;let $=h[e].max-h[e].min;$=$?h[e].max/$:1,$=u.bottom*$,i[w].gradient=i.createLinearGradient(0,$,0,u.top),a.forEach(S=>i[w].gradient.addColorStop(S.offset,S.color))}return i[w].gradient}}function _(){return{labels:n.map(m),datasets:[{label:"Personal Rating",data:n.map(p=>p.personalRating?p.personalRating*10:null),yAxisID:"yAxisRating",borderColor:p=>x(p,"_ratingPersonal","yAxisRating",{offset:0,color:"rgb(97 97 97 / 0.6)"},{offset:.1,color:"rgb(97 97 97 / 0.6)"},{offset:1,color:"rgb(175 2 0)"}),backgroundColor:p=>x(p,"_ratingPersonal","yAxisRating",{offset:0,color:"rgb(97 97 97 / 0.6)"},{offset:.1,color:"rgb(97 97 97 / 0.6)"},{offset:1,color:"rgb(175 2 0)"})},{label:"General Rating",data:n.map(p=>p.generalRating),yAxisID:"yAxisRating",fill:{target:"-1",above:`rgb(255 0 0 / ${t("body").hasClass("dark-knight")?.15:.3})`,below:`rgb(0 255 0 / ${t("body").hasClass("dark-knight")?.15:.3})`},borderColor:p=>x(p,"_ratingGeneral","yAxisRating",{offset:0,color:"rgb(97 97 97 / 0.6)"},{offset:.1,color:"rgb(97 97 97 / 0.6)"},{offset:1,color:"rgb(225 31 117)"}),backgroundColor:p=>x(p,"_ratingGeneral","yAxisRating",{offset:0,color:"rgb(97 97 97 / 0.6)"},{offset:.1,color:"rgb(97 97 97 / 0.6)"},{offset:1,color:"rgb(225 31 117)"})},{label:"Watchers",data:n.map(p=>p.watchers),yAxisID:"yAxisWatchers",borderColor:p=>x(p,"_watchers","yAxisWatchers",{offset:0,color:"rgb(154 67 201 / 0.2)"},{offset:1,color:"rgb(154 67 201)"}),backgroundColor:p=>x(p,"_watchers","yAxisWatchers",{offset:0,color:"rgb(154 67 201 / 0.2)"},{offset:1,color:"rgb(154 67 201)"})},{label:"Comments",data:n.map(p=>p.comments),yAxisID:"yAxisComments",borderColor:p=>x(p,"_comments","yAxisComments",{offset:0,color:"rgb(54 157 226 / 0.2)"},{offset:1,color:"rgb(54 157 226)"}),backgroundColor:p=>x(p,"_comments","yAxisComments",{offset:0,color:"rgb(54 157 226 / 0.2)"},{offset:1,color:"rgb(54 157 226)"})}]}}function y(){return{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"nearest",axis:"x",intersect:!1},animation:{delay:p=>p.type==="data"&&p.mode==="default"?(d?500:0)+p.dataIndex*(750/Math.max(n.length-1,1))+p.datasetIndex*100:0},scales:{x:{offset:!0},yAxisRating:{type:"linear",position:"left",offset:!0,suggestedMin:60,max:100,title:{display:!0,text:"Rating"},grid:{color:p=>p.tick.value%10?Chart.defaults.borderColor:"rgb(55 55 55)"},ticks:{callback:p=>`${p}%`}},yAxisWatchers:{type:"linear",position:"right",offset:!0,min:0,suggestedMax:10,title:{display:!0,text:"Watchers"},grid:{drawOnChartArea:!1},ticks:{callback:p=>s.formatTLC(p)}},yAxisComments:{type:"linear",position:"right",offset:!0,min:0,suggestedMax:10,title:{display:!0,text:"Comments"},grid:{drawOnChartArea:!1}}},plugins:{tooltip:{usePointStyle:!0,boxPadding:5,backgroundColor:"rgb(0 0 0 / 0.5)",caretSize:10,padding:{x:18,y:6},titleFont:{size:13,weight:"bold"},callbacks:{title:p=>{let w=n[p[0].parsed.x].mainTitle;return w=w.length>20?w.slice(0,20).trim()+"...":w,`${p[0].label}${w?`
${w}`:""}`},label:p=>{const w=p.parsed.x,e=p.parsed.y,a=unsafeWindow.userscriptAvgSeasonEpisodeRatings;return p.datasetIndex===0?`${e/10}${a?.personal?.average?`  (avg: ${a.personal.average.toFixed(1)})`:""}`:p.datasetIndex===1?`${e}%  (${s.formatTLC(n[w].votes)} v.)${a?.general?`  (avg: ${a.general.average?Math.round(a.general.average):"--"}%)`:""}`:p.datasetIndex===2?`${s.formatTLC(e)}${n[0].watchers?`  (${Math.round(e*100/n[0].watchers)}%)`:""}`:`${e}`},labelColor:p=>({borderColor:p.dataset.borderColor(),backgroundColor:p.dataset.backgroundColor()}),footer:p=>{const w=n[p[0].parsed.x].releaseDate;return w?unsafeWindow.formatDate?.(w)||w:void 0}}},legend:{labels:{usePointStyle:!0,filter:(p,w)=>w.datasets[p.datasetIndex].data.some(e=>e!==null)}},filler:{propagate:!1},zoom:{zoom:{mode:"x",drag:{enabled:!0,threshold:0}},limits:{x:{minRange:8}}}}}}function k(){GM_addStyle(`
...
#summary-wrapper a:has(> .certification):hover {
  ...
  `)}})("Trakt.tv | Enhanced Title Metadata"),gmStorage.kji85iek&&(async C=>{"use strict";let t;v(),document.addEventListener("turbo:load",()=>{t??=unsafeWindow.jQuery,t&&(unsafeWindow.ratingOverlay=r,f(),t(document).off("ajaxSuccess.userscript12944").on("ajaxSuccess.userscript12944",(c,m,o)=>{o.url.endsWith("/popular_lists")&&(f(),unsafeWindow.addOverlays())}))},{capture:!0});function r(c,m){if(!c.length){const o=c.end();o.closest(".personal-list").length&&o.hasClass("poster")&&(c=o)}c.find(".corner-rating").length||c.prepend(`<div class="corner-rating corner-rating-${m}"><div class="text">${m}</div></div>`)}function f(){t(".personal-list .poster[data-url]:not(:has(> a))").each(function(){t(this).children().wrapAll(`<a href="${t(this).attr("data-url")}"></a>`)})}unsafeWindow.userscriptAddLinksToListPreviewPosters=f;function v(){GM_addStyle(`
...
.personal-list .poster.dropped-show .dropped-badge-wrapper {
  ...
  `)}})("Trakt.tv | Enhanced List Preview Posters"),gmStorage.p2o98x5r&&(async C=>{"use strict";t(),document.addEventListener("turbo:load",()=>{if(!/^\/users\/[^\/]+\/lists$/.test(location.pathname))return;const r=unsafeWindow.jQuery;if(!r)return;const f=r("#sortable-grid"),v=f.children().length?r('<hr id="all-in-one-lists-view-spacer">').insertAfter(f):void 0,c=r('<button id="all-in-one-lists-view-btn" type="button">All-In-One Lists View</button>').insertAfter(v??f);c.on("click",async()=>{c.text("Loading...").prop("disabled",!0);const m=async l=>fetch(location.pathname+l).then(b=>b.text()).then(b=>r(new DOMParser().parseFromString(b,"text/html")).find(".personal-list"));let o=r((await Promise.all(["/collaborations","/liked","/liked/official"].map(m))).flatMap(l=>l.get()));const n=r(".personal-list"),d=n.map((l,b)=>r(b).attr("data-list-id")).get();if(o=o.filter((l,b)=>!d.includes(r(b).attr("data-list-id"))),!o.length){c.text("No other lists found.");return}const s=+n.last().attr("data-rank");o.each((l,b)=>r(b).attr("data-rank",s+l+1)),o.find(".btn-list-progress").click(function(){unsafeWindow.showLoading();const l=r(this).attr("data-list-id");l&&unsafeWindow.userSettings?.user.vip?unsafeWindow.redirect(unsafeWindow.userURL("progress?list="+l)):unsafeWindow.redirect("/vip/list-progress")}).end().find(".btn-list-subscribe").click(function(){unsafeWindow.showLoading();const l=r(this).attr("data-list-id");l&&unsafeWindow.userSettings?.user.vip?r.post(`/lists/${l}/subscribe`,function(b){unsafeWindow.redirect(b.url)}).fail(function(){unsafeWindow.hideLoading(),unsafeWindow.toastr.error("Doh! We ran into some sort of error.")}):unsafeWindow.redirect("/vip/calendars")}).end().find(".collaborations-deny").on("ajax:success",function(l,b){r("#collaborations-deny-"+b.id).children().addClass("trakt-icon-delete-thick"),r("#collaborations-approve-"+b.id).addClass("off"),r("#collaborations-block-"+b.id).addClass("off")});const g=r("#btn-list-edit-lists");g.hasClass("active")&&g.trigger("click"),g.hide(),f.append(o),v?.remove(),c.remove(),unsafeWindow.genericTooltips(),unsafeWindow.vipTooltips(),unsafeWindow.shareIcons(),unsafeWindow.convertEmojis(),unsafeWindow.userscriptAddLinksToListPreviewPosters?.(),unsafeWindow.addOverlays(),unsafeWindow.$grid?.isotope("insert",o),unsafeWindow.updateListsCount(),unsafeWindow.lazyLoadImages(),unsafeWindow.renderReadmore()})},{capture:!0});function t(){GM_addStyle(`
...
body:has(#btn-list-edit-lists.active) :is(#all-in-one-lists-view-btn, #all-in-one-lists-view-spacer) {
  ...
  `)}})("Trakt.tv | All-In-One Lists View"),gmStorage.pmdf6nr9&&(async C=>{"use strict";let t,r;const f=new Intl.NumberFormat("en-US",{notation:"compact",maximumFractionDigits:1});f.formatTLC=s=>f.format(s).toLowerCase(),d(),document.addEventListener("turbo:load",async()=>{if(!/^\/(shows|movies)\//.test(location.pathname)||(t??=unsafeWindow.jQuery,r??=unsafeWindow.userscriptTraktApiWrapper,!t))return;const s=t("#summary-wrapper"),g=s.find("#summary-ratings-wrapper"),l=g.find(".trakt-rating > a").attr("href");if(!l)return;const b=t('<div id="ratings-distribution-chart-wrapper"><canvas></canvas></div>').appendTo(s.find(".shadow-base")).find("canvas"),[x,_]=await Promise.all([v(l),c(s)]),y=()=>{new Chart(b[0].getContext("2d"),{type:"bar",data:o(x,_),options:n(x,g)})};document.hidden?t(document).one("visibilitychange",y):y()},{capture:!0});async function v(s){let g;if(r){const l=s.split("/").slice(1,-1),b=isNaN(l[1])?l[1]:t(".summary-user-rating").attr(`data-${l[0].slice(0,-1)}-id`),x=await r[l[4]??l[2]??l[0]].ratings({id:b,season:l[3],episode:l[5]});g={distribution:Object.values(x.distribution),votes:x.votes}}else{const l=await fetch(s),b=new DOMParser().parseFromString(await l.text(),"text/html");g={distribution:JSON.parse(t(b).find("#charts-wrapper script").text().match(/ratingsDistribution = (\[.*\])/)[1]),votes:t("#summary-ratings-wrapper").data("vote-count")}}return g.distribution.length===11&&(console.warn(GM_info.script.name.replace("Trakt.tv","Userscript")+": Malformed ratings distribution data.",g.distribution.toString()),g.distribution.shift()),g}function c(s){const g=s.find("> .full-screenshot"),l=async()=>{const b=g.css("background-image").match(/https.*webp/)?.[0];if(!b)return .5;const x=await GM.xmlHttpRequest({url:b,responseType:"blob",fetch:!0});if(x.status!==200)throw new Error(`XHR for: ${x.finalUrl} failed with status: ${x.status}`);const _=URL.createObjectURL(x.response),y=new Image;y.src=_,await y.decode(),URL.revokeObjectURL(_);const k=document.createElement("canvas");k.width=y.naturalWidth,k.height=y.naturalHeight;const p=k.getContext("2d");p.drawImage(y,0,0);const w=y.naturalWidth/4,e=y.naturalHeight/4,a=p.getImageData(3*w,2*e,w,e).data;let i=0,u=a.length/16;for(let h=0;h<a.length;h+=16)i+=(.299*a[h]+.587*a[h+1]+.114*a[h+2])/255;return i/u};return g.attr("style")?l():new Promise(b=>{new MutationObserver((x,_)=>{_.disconnect(),b(l())}).observe(g[0],{attributeFilter:["style"]})})}function m(s,g,l,...b){if(!s)return b.pop().color;const{ctx:x,chartArea:_,scales:y}=s.chart;if(_){if(x[g]??={},!x[g].gradient||x[g].height!==_.height||x[g].yAxisMin!==y[l].min||x[g].yAxisMax!==y[l].max){let k=y[l].max-y[l].min;k=k?y[l].max/k:1,k=_.bottom*k,x[g].gradient=x.createLinearGradient(0,k,0,_.top),b.forEach(p=>x[g].gradient.addColorStop(p.offset,p.color)),x[g].height=_.height,x[g].yAxisMin=y[l].min,x[g].yAxisMax=y[l].max}return x[g].gradient}}function o(s,g){return{labels:[...Array(10)].map((l,b)=>String(b+1)),datasets:[{label:"Votes",data:s.distribution,categoryPercentage:1,barPercentage:.97,backgroundColor:`rgba(${Array(3).fill(Math.min(g+.35,1)*255).join(", ")}, ${Math.min(g+.3,.7)})`,hoverBackgroundColor:l=>m(l,"_votes","y",{offset:0,color:`rgba(155, 66, 200, ${Math.min(g+.3,.7)})`},{offset:.9,color:`rgba(255, 0, 0, ${Math.min(g+.3,.7)})`})}]}}function n(s,g){return{responsive:!0,maintainAspectRatio:!1,minBarLength:2,interaction:{mode:"index",intersect:!1},animation:{delay:l=>l.type==="data"&&l.mode==="default"?250+l.dataIndex*(750/(s.distribution.length-1)):0},scales:{x:{display:!1},y:{display:!1,suggestedMax:10}},plugins:{tooltip:{displayColors:!1,backgroundColor:"rgba(0, 0, 0, 0.7)",caretSize:10,padding:{x:12,y:5},titleAlign:"center",titleMarginBottom:2,titleFont:{weight:"bold"},bodyAlign:"center",bodyColor:"rgb(170, 170, 170)",bodyFont:{size:11},footerAlign:"center",footerColor:l=>`hsl(0, ${l.tooltip.dataPoints[0].parsed.x*11}%, 35%)`,footerMarginTop:2,footerFont:{size:18},callbacks:{title:l=>{const b=l[0].label;return`${b} - ${unsafeWindow.ratingsText?.[b]}`},label:l=>{const b=l.parsed.y;return`${s.votes>0?(b*100/s.votes).toFixed(1):"--"}% (${f.formatTLC(b)} v.)`},footer:l=>{const b=g.find('.summary-user-rating > :not([style="display: none;"]) > [class*="rating-"]').first().attr("class")?.match(/rating-(\d+)/)?.[1];return l[0].parsed.x===b-1?"\u2764":""}}},legend:{display:!1}},onClick:(l,b)=>{if(!b.length)return;const x=b[0].index+1;g.find(".summary-user-rating:not(.popover-on)").trigger("click"),setTimeout(()=>t(`.needsclick.rating-${x}`).trigger("mouseover").trigger("click"),500)}}}function d(){GM_addStyle(`
#summary-wrapper {
  container-type: inline-size;
...
${f.concat(v).map(e=>e.addStyles).filter(Boolean).join(`
`)}
  `)}})("Trakt.tv | Custom Links (Watch-Now + External)"),gmStorage.x70tru7b&&(async C=>{"use strict";let t,r,f;const v={_defaults:{title:(typeof C<"u"?C:GM_info.script.name).replace("Trakt.tv","Userscript"),toast:!0,toastrOpt:{positionClass:"toast-top-right",timeOut:1e4,progressBar:!0},toastrStyles:"#toast-container#toast-container a { color: #fff !important; border-bottom: dotted 1px #fff; }"},_print(l,b,x="",_={}){const{title:y=this._defaults.title,toast:k=this._defaults.toast,toastrOpt:p,toastrStyles:w="",consoleStyles:e="",data:a}=_,i=`${x}${a!==void 0?" See console for details.":""}<style>${this._defaults.toastrStyles+w}</style>`;console[l](`%c${y}: ${x}`,e,...a!==void 0?[a]:[]),k&&unsafeWindow.toastr?.[b](i,y,{...this._defaults.toastrOpt,...p})},info(l,b){this._print("info","info",l,b)},success(l,b){this._print("info","success",l,{consoleStyles:"color:#00c853;",...b})},warning(l,b){this._print("warn","warning",l,b)},error(l,b){this._print("error","error",l,b)}},c={...GM_getValue("vipUnlock")};GM_setValue("vipUnlock",c);const m=null;g(),document.addEventListener("turbo:load",async()=>{if(t??=unsafeWindow.jQuery,r??=unsafeWindow.compressedCache,f??=unsafeWindow.Cookies,!(!t||!r||!f)&&(t("body").removeAttr("data-turbo"),o(),m&&t("body:not(.dashboard) .feed-icon.csv").attr("href",location.pathname+".csv?slurm="+m+location.search.replace("?","&")),t(document).off("ajaxSuccess.userscript38793").on("ajaxSuccess.userscript38793",(l,b,x)=>{x.url.endsWith("/settings.json")&&o(),m&&/\/dashboard\/(on_deck|recently_watched)$/.test(x.url)&&t('.feed-icon.csv[href="/vip/csv"]').attr("href",function(){return t(this).prev().attr("data-path")+".csv?"+["slurm="+m,t(this).prev().attr("data-query")].join("&")})}),t(".frame-wrapper .sidenav.advanced-filters .buttons").addClass("vip").find(".btn.vip").text("").removeClass("vip").removeAttr("href").addClass("disabled disabled-init").attr("id","filter-apply").attr("data-apply-text","Apply Filters").before('<a class="btn btn-close-2024" id="filter-close" style="display: inline-block !important; visibility: visible !important;">Close</a>').append('<span class="text">Configure Filters</span><span class="icon fa-solid fa-check"></span>'),/^\/users\/[^\/]+\/progress(?!\/playback)/.test(location.pathname)&&/list=\d+/.test(location.search)&&!location.search.includes("terms="))){unsafeWindow.showLoading?.();const l=new URLSearchParams(location.search),b=l.get("list"),x=await fetch("/lists/"+b).then(y=>fetch(y.url+"?display=show&hide=unwatched&limit=10000")).then(y=>y.text()).then(y=>new DOMParser().parseFromString(y,"text/html")),_=[...x.querySelectorAll(".grid-item")].map(y=>y.querySelector(".titles-link")?.textContent).filter(Boolean);l.set("terms",`^${_.join("$|^")}$`),["airing","completed","ended","not-completed","rewatching"].forEach(y=>{f.remove("filter-hide-progress-"+y,{path:"/"}),f.remove("filter-hide-progress-"+y,{path:"/users/"+f.get("trakt_userslug")})}),location.search=l.toString()}},{capture:!0});function o(){const l=r.get("settings");l&&(!l.user.vip||m&&l.account.token!==m)&&(l.user.vip=!0,m&&(l.account.token=m),r.set("settings",l),unsafeWindow.userSettings&&(unsafeWindow.userSettings=l))}async function n(){if(unsafeWindow.listPopupPressed){unsafeWindow.listPopupPressed=!1;return}const l=t(this).hasClass("side-btn")||t(this).parent().hasClass("side-btn"),b=t(this).hasClass("btn-list"),x=l?t(this).closest(".btn-summary"):b?t(this):t(this).closest(".grid-item"),_=x.attr("data-url"),y=x.attr("data-type"),k=+x.attr(`data-${y}-id`),p=Object.values(r.get("lists")??{}).some(e=>e.type==="list"),w=unsafeWindow.isPersonPage&&b||x.attr("data-type")==="person"?"list":unsafeWindow.userSettings.browsing.list_popup_action;if(unsafeWindow.isPersonPage&&b||p&&(w!=="watchlist"||t(this).hasClass("selected"))||l)unsafeWindow.actionListPopup(l?x:t(this));else{x.find(".loading").show();const e=t(this).hasClass("selected"),a=Object.values(r.get("lists")).find(h=>h.name==="Watchlist"),i=t(`<li class="${a.item_count>=unsafeWindow.userSettings.limits.watchlist.item_count?"maxed-out":""} ${e?"selected":""}" data-list-id="${a.ids.trakt}" data-list-type="watchlist" data-item-count="${a.item_count}"></li>`);await d(x,i,e)&&(t(`[data-${y}-id="${k}"]:is(.btn-summary.btn-list, [data-type="${y}"]) .list`)[e?"removeClass":"addClass"]("selected"),unsafeWindow.cacheUserData()),x.find(".loading").hide()}}async function d(l,b,x){b.addClass("spinner").find(".icon").addClass("fa-spin");const _=l.attr("data-url"),y=l.attr("data-type"),k=+l.attr(`data-${y}-id`),p=+b.attr("data-list-id")||Object.values(r.get("lists")).find(a=>a.name==="Watchlist").ids.trakt,w=b.attr("data-list-type"),e=+b.attr("data-item-count");try{if(b.hasClass("maxed-out")&&!x){const a=.045*e;v.info(`Target list is maxed-out, attempting bypass.. This will take about <strong>${~~(a/60)}m${~~(a%60)}s</strong>.`,{toastrOpt:{timeOut:a*1e3}});const i=await s(),u=r.get("lists");if(u[i]&&u[i].item_count>0){v.error("Temp list is not empty. Aborting..",{data:u[i]});return}const h=await fetch(_+"/list",{method:"POST",headers:{"X-CSRF-Token":unsafeWindow.csrfToken},body:new URLSearchParams({type:y,trakt_id:k,list_id:i})});if(!h.ok){v.error(`Failed to add item to temp list: id=${i}.`,{data:h});return}v.info("Added item to temp list.");for(const[$,S]of[[p,i],[i,p]]){const T=await fetch("/lists/"+$).then(P=>P.text()).then(P=>new DOMParser().parseFromString(P,"text/html").querySelector("#listable-all-item-ids").value.split(",").map(Number));if(!T||!T.length){v.error(`Failed to fetch all list item ids for list: id=${$}.`);return}const E=await fetch(`/lists/${$}/move_items/${S}`,{method:"POST",headers:{"X-CSRF-Token":unsafeWindow.csrfToken},body:new URLSearchParams([["sort_by","rank"],["sort_how","asc"],...T.map(P=>["order[]",P])])});if(!E.ok){v.error(`Failed to move all items from ${$===p?"target to temp":"temp to target"} list.`,{data:E});return}v.info(`Moved all items from ${$===p?"target to temp":"temp to target"} list.`)}v.success(`Success. Item was added to <a href="/lists/${p}"><strong>target list</strong></a>.`)}else{const a=await fetch(`${_}/${/(watchlist|favorites|recommendations)/.test(w)?w:"list"}${x?"/remove":""}`,{method:"POST",headers:{"X-CSRF-Token":unsafeWindow.csrfToken},body:new URLSearchParams({type:y,trakt_id:k,list_id:p})});if(!a.ok){await a.json().then(i=>v.error("Failed to add item to list."+(i.message?" Response: "+i.message:""),{data:a}));return}v.success("Success. "+(await a.json()).message)}return b.toggleClass("selected"),!0}finally{b.removeClass("spinner").find(".icon").removeClass("fa-spin")}}async function s(){if(!c.tempList1Id||!r.get("lists")[c.tempList1Id]){const l=Object.values(r.get("lists")).find(y=>y.name==="Favorites").ids.trakt,b=await fetch(`/lists/${l}/copy_items/0`,{method:"POST",headers:{"X-CSRF-Token":unsafeWindow.csrfToken},body:new URLSearchParams({"order[]":"",sort_by:"rank",sort_how:"asc"})});if(!b.ok){v.error("Failed to create temp list.",{data:b});return}const x=(await b.json()).id;v.info(`Created temp list: id=${x}.`,{data:b});const _=await fetch("/lists/"+x,{method:"POST",headers:{"X-CSRF-Token":unsafeWindow.csrfToken},body:new URLSearchParams({authenticity_token:unsafeWindow.csrfToken,_method:"put",name:`temp1_${x}`,description:'Needed for the list limits bypass of the "Partial VIP Unlock" userscript. Keep it empty. You can edit the list title and description if you want. If you delete it another one will be created on the next attempted list limits bypass.',privacy_hidden:"private",privacy:"private",existing_collaborator_ids:"",allow_comments_hidden:1,allow_comments:1,display_numbers_hidden:1,display_numbers:1,default_sort_by:"rank",default_sort_how:"asc"})});if(!_.ok){v.error("Failed to update temp list metadata.",{data:_});return}v.info("Updated temp list metadata.",{data:_}),c.tempList1Id=x,GM_setValue("vipUnlock",c)}return c.tempList1Id}function g(){GM_addStyle(`
#top-nav .btn-vip,
...
.personal-list .comment-wrapper .user-name [href="/users/${l}"] {
  white-space: nowrap;
    ...
    `)}})("Trakt.tv | Partial VIP Unlock"),gmStorage.yl9xlca7&&(async C=>{"use strict";let t;const r=new Intl.NumberFormat("en",{notation:"compact",maximumFractionDigits:1});r.formatTLC=n=>r.format(n).toLowerCase(),o(),document.addEventListener("turbo:load",()=>{if(!location.pathname.startsWith("/shows/")||location.pathname.includes("/episodes/")||(t??=unsafeWindow.jQuery,!t))return;const n=t("#seasons-episodes-sortable"),d=t("#summary-ratings-wrapper .summary-user-rating"),s=t("#summary-ratings-wrapper .trakt-rating");if(!n.length||!d.length||!s.length)return;const g=unsafeWindow.userscriptAvgSeasonEpisodeRatings={};let l;d[0].mutObs=new MutationObserver(()=>{d.hasClass("popover-on")||c(d,g.personal)}),c(d),m(s);const b=!location.pathname.endsWith("/seasons/0");n.on("arrangeComplete",()=>{n.data("isotope")&&(l=n.data("isotope").filteredItems.filter(x=>b?x.element.dataset.seasonNumber!=="0":!0),g.personal=f(l),g.general=v(l),c(d,g.personal),m(s,g.general))}),t(document).off("ajaxSuccess.userscript32985").on("ajaxSuccess.userscript32985",(x,_,y)=>{l&&/\/ratings\/(seasons|episodes)\.json$|\/rate/.test(y.url)&&(g.personal=f(l),c(d,g.personal))})},{capture:!0});function f(n){const d=n.map(s=>+t(s.element).find(".corner-rating > .text").text()).filter(Boolean);return{average:d.length?d.reduce((s,g)=>s+g,0)/d.length:void 0,votes:d.length}}function v(n){const d=n.reduce((s,g)=>s+g.sortData.votes,0);return{average:d?n.reduce((s,g)=>s+g.sortData.percentage*(g.sortData.votes/d),0):void 0,votes:d}}function c(n,d){n[0].mutObs.disconnect(),n.find(".rating").each(function(){const s=t(this).parent().prev().attr("class").match(/rating-(\d+)/)?.[1];s&&t(this).html(`${s}<div class="votes">${unsafeWindow.ratingsText?.[s]??""}</div>`)}),n.find(".number > .votes").removeClass("alt").text(`avg: ${d?.average?`${d.average.toFixed(1)}`:"--"} (${d?.votes!==void 0?r.formatTLC(d.votes):"--"} v.)`),n[0].mutObs.observe(n[0],{subtree:!0,childList:!0})}function m(n,d){n.has(".rating .votes").length||n.find(".votes").clone().appendTo(n.find(".rating")).text((s,g)=>`(${g.match(/^.*? v/)?.[0]??"0 v"}.)`),n.find(".number > .votes").text(`avg: ${d?.average?`${Math.round(d.average)}`:"--"}% (${d?.votes!==void 0?r.formatTLC(d.votes):"--"} v.)`)}function o(){GM_addStyle(`
